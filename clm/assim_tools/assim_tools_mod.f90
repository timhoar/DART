...
use      types_mod,       only : r8, digits12, PI, missing_r8, NAN
...
integer  :: my_num_obs, i, j, k, ispc, owner, owners_index, my_num_state
...
!PAR URGENT: MAKE SURE THIS INDEXING IS CORRECT; SAME FOR OTHER COMP_COV_FACTOR
      ...
      ! TJH debug FIXME ultimately check ens_mean
...
! TJH could be way off, but if there are some missing values in the state
!     we cannot just include them in the math ...

if (any(state == MISSING_R8)) then
   state_inc = 0.0_r8
   reg_coef  = 0.0_r8
   ! net_a      will be unchanged
   ! correl_out will be unchanged
   return
endif
