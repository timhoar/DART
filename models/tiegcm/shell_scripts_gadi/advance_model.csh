...
#   set RUN_CMD = ''
#   set RUN_CMD = 'mpirun -np 1 --bind-to none'
   set mpirun = `cat ../mpirun.command`
   set RUN_CMD = $mpirun' -np 1 --bind-to none'
endif
...
while($state_copy <= $num_states)
   ...
   # Copy across data for Weimar model required by TIE-GCM 2.0
   cp -p  ../wei05sc.nc  wei05sc.nc          >>&   $logfile || exit 2
   ...
   #$mpirun -np $nproc ../tiegcm tiegcm.nml >>& $logfile
