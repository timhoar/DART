...
real(r8), allocatable :: zGridFull(:,:)   ! geometric height at cell faces   (nVertLevelsP1,nCells)
...
! this reads in latCell, lonCell, zGridFull, cellsOnVertex
call get_grid()

! read in vert cell face locations and then compute vertical center locations
do kloc=1, nCells
 do iloc=1, nVertLevels
   zGridCenter(iloc,kloc) = (zGridFull(iloc,kloc) + zGridFull(iloc+1,kloc))*0.5_r8
 enddo
enddo
...
if (highest_obs_pressure_mb > 0.0) then
   ...
   if(vert_is_pressure(location)) then
      ...
      new_location = location
      call vert_convert(ens_mean, new_location, obs_type, VERTISPRESSURE, istatus)
      if(istatus == 0) then
...
!print *, 'got to end of model_interpolate with no case, kind = ', obs_type
! shouldn't get here.
interp_val = MISSING_R8
istatus = 100
...
!print *, 'got ivar ', ivar(1), ' from kind ', obs_kinds(i)
   if (ivar(1) <= 0) then
      ...
      if (obs_kinds(i) == KIND_TEMPERATURE) cycle oktointerp
...
if (debug > 9) print *, 'requesting interpolation at ', llon, llat, lheight


!  if (debug > 2) print *, 'base offset now ', base_offset(1)

! the logic here depends on some module global variables.  at some point
! make decisions about the final options and remove the extra logic.
...
!print *, 'called compute_w_bary, kind, istatus: ', obs_kinds(i), istatus
      if (istatus /= 0) return
...
!print *, 'at end of local_interp loop, istatus = ', istatus
...
if ( output_state_vector ) then
   ...
   call nc_check(nf90_put_var(ncFileID, VarID, zGridFull ), &
...
!   latCell, lonCell, zGridFull, cellsOnVertex (all in module global storage)
...
call nc_check(nf90_get_var( ncid, VarID, zGridFull), &
...
! Initialization
ier = 0           ! default error return is 0
fract = 0.0_r8
lower = 1
upper = nVertLevels
...
if(height < bounds(1) .or. height > bounds(nbounds)) then
   !JLA 
   if(debug > 5 .and. my_task_id() == 0) write(*, '(A,I5,3F10.2)') &
   'fail in find_height_bounds ', ier, height, bounds(1), bounds(nbounds)
   return
endif
...
if(vert_is_level(loc)) then
   ...
   if(debug > 9) print *, 'find_vert_level for level: vert, lower, upper, fract = ', &
                           vert, lower, upper, fract
   ier = 0
   return
endif
   ...
   do i=1, nc
      if (oncenters) then
         call find_height_bounds(vert, nVertLevels, zGridCenter(:, ids(i)), &
                                 lower(i), upper(i), fract(i), ier)
         if(debug > 9 .and. my_task_id() == 0) print '(A,3I5,F10.4,I4,3F10.4)', &
         'after find_height_bounds: ier, i, cellid, vert, lower, fract, Z_low, Z_up, ier: = ', &
         ier, i, ids(i), vert, lower(i), fract(i), zGridCenter(lower(i),ids(i)), zGridCenter(upper(i),ids(i))
         ...
         if(debug > 9 .and. my_task_id() == 0) print '(A,3I5,F10.4,I4,3F10.4)', &
         'after find_height_bounds: ier, i, cellid, vert, lower, fract, Z_low, Z_up, ier: = ', &
         ier, i, ids(i), vert, lower(i), fract(i), zGridEdge(lower(i),ids(i)), zGridEdge(upper(i),ids(i))

      endif
      if(debug > 5 .and. ier /= 0 .and. my_task_id() == 0) print '(A,4I5,F10.4,I4,F10.4)', &
        'fail in find_vert_level: ier, nc, i, id, vert, lower, fract: ', &
         ier, nc, i, ids(i), vert, lower(i), fract(i)
      if (ier /= 0) return
...
if(p > pressure(1) .or. p < pressure(nbounds)) return
...
! Check if the pressure at the lower level is higher than at the higher level.
do i = 2, nbounds
   call get_interp_pressure(x, pt_base_offset, density_base_offset, qv_base_offset, &
        cellid, i, nbounds, pressure(i), gip_err)
   if(pressure(i) > pressure(i-1)) then
      if(debug>9) print*,"ALARM! find_pressure_bounds: p_lower > p_higher ",&
                         cellid,i,pressure(i),pressure(i-1)
   endif
enddo
...
!print *, 'find p bounds i, pr(i) = ', i, pressure(i), gip_err
   ...
   if(p > pressure(i)) then
      ...
      if(debug>9 .and. my_task_id() == 0) print '(A,3F12.4,2I4,F22.18)', &
...
if(ier /= 0) then
   if(debug > 5 .and. my_task_id() == 0) then
      llv = get_location(loc)
      verttype = nint(query_location(loc))
      write(string1,'("lon, lat, vert, which_vert:",3F20.9,I5)') llv(1:3), verttype
      call error_handler(E_MSG, 'compute_scalar_with_barycentric',string1,source, revision, revdate)
      print *,'failure from find_triangle_vert_indices: ier = ',ier,' nc = ',nc, ' c = ',c(1:nc)
   endif
   return
endif
if(debug > 9 .and. nc == 1 .and. my_task_id() == 0) then
   llv = get_location(loc)
   verttype = nint(query_location(loc))
   print*,'after find_triangle_vert_indices: c =',c(1:nc),' loc =',llv(1:3),verttype
   !print*,'compute_scalar_with_barycentric: c =',c(1:nc),' loc =',llv(1:3),verttype
endif
...
do k=1, n
   ...
   do i = 1, nc
      ...
      dval(k) = dval(k) + weights(i) * fdata(i)
   enddo
...
!print *, 'returning because cell on boundary'
   ier = 12
   return
endif

if (.not. inside_cell(cellid, lat, lon)) then
!print *, 'returning because not in cell'
   ier = 13
   return
endif
...
if(debug > 9)  then
write(string2,'("cellid, closest_vert:",2I10)') cellid, closest_vert
!print *, trim(string2)
!call error_handler(E_MSG, 'find_triangle_vert_indices',string2, source, revision, revdate)
endif
...
!if (all(t1 == r)) then     ! Located at a grid point
...
!if(ier /= 0 .and. my_task_id() == 0) print*, 't1 /= r: ier from find_vert_level = ',ier
if(ier /= 0) return
...
!   print *, celllist(1:ncells)
   call find_vert_level(x, loc, ncells, celllist, .true., lower, upper, fract, ier)
   if (ier /= 0) return
   ...
   ! need vert index for the vertical level
   call find_vert_level(x, loc, nedges, edgelist, .false., lower, upper, fract, ier)
   if (ier /= 0) return
endif
...
if (closest_vertex_ll < 0) then
   if (debug > 8) print *, 'cannot find nearest vertex to lon, lat: ', lon, lat
endif
...
if (all (abs(r - t1) < roundoff)) then   ! Located at a grid point (counting roundoff errors)
!if (all(abs(r - t1) < 0.0_r8)) then
   inside = .true.
...
if (any(weights < -1.0e-12_r8)) then
   !print *, 'inside_triangle() by weights: outside'
   !print *, weights
   inside = .false.
   weights = 0.0_r8
   return
...
!call intersect_triangle(p(:,1), p(:,2), p(:,3), s, intp)
call error_handler(E_ERR, 'latlon_to_xyz_on_plane', 'not implemented for intersection', &
                  source, revision, revdate)
...
!print '(A,5F15.5)', 'compute_full_pressure: ', theta, rho, qv_nonzero, tk, pressure
...
select case (ztypeout)
   ...
   !print *, 'ready to call find_triangle_vert_indices'
   !print *, 'x = ', x, ' n = ', n
   call find_triangle_vert_indices (x, location, n, c, k_low, k_up, fract, weights, istatus)
   if(debug > 5 .and. istatus /= 0 .and. my_task_id() == 0) then
      write(string2,'("ier from find_triangle_vert_indices:",I3," n: ",I3, &
            " cell_ids: ",3I8," vert: ",f8.2," k_low: ",3f8.2," fract: ",3f8.2)') &
            istatus,n,c,zin,k_low,fract
   ...
   call find_triangle_vert_indices (x, location, n, c, k_low, k_up, fract, weights, istatus)
   if(debug > 5 .and. istatus /= 0 .and. my_task_id() == 0) then
      write(string2,'("istatus from find_triangle_vert_indices:",I5," cell_ids: ",3I10)') istatus,c
      call error_handler(E_MSG, 'vert_convert',string2,source, revision, revdate)
   endif
   if (istatus /= 0) return
