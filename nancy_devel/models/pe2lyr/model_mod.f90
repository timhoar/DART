...
! not needed; pi is defined in the types module.
!pi = 4.*atan(1.0)
...
u_indxmax =   nlevs*nlats*nlons
v_indxmax = 2*nlevs*nlats*nlons
z_indxmax = 3*nlevs*nlats*nlons
...
if(mytype == KIND_U_WIND_COMPONENT) then
   ...
   indx =   nlevs*nlats*nlons+(level-1)*nlats*nlons+(lat_index-1)*nlons + lon_index
else if(mytype == KIND_GEOPOTENTIAL_HEIGHT) then
   indx = 2*nlevs*nlats*nlons+(level-1)*nlats*nlons+(lat_index-1)*nlons + lon_index
endif
