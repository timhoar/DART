...
use         obs_kind_mod, only : get_num_raw_obs_kinds, get_obs_kind_index,               &
...
character(len = 129)   :: errstring, errstring2, errstring3
...
! a single cutoff value.  note that these are generic kinds 
! (e.g. TEMPERATURE and not RADIOSONDE_TEMPERATURE) because that's
! what get_close() is passed.
num_kinds = get_num_raw_obs_kinds()
allocate(cutoff_list(0:num_kinds)) 
cutoff_list(:) = cutoff
...
call update_copies(ens_handle)
call update_copies(obs_ens_handle)
...
!PAR NEED TO TURN STUFF OFF MORE EFFICEINTLY
   ...
   OBS_UPDATE: do j = 1, num_close_obs
      ...
      if(my_obs(obs_index) > i) then
         ...
         obs_qc = obs_ens_handle%copies(OBS_GLOBAL_QC_COPY, obs_index)
         if (nint(obs_qc) > 1) cycle OBS_UPDATE
...
if ((obs_var == 0.0_r8) .and. (prior_var == 0.0_r8)) then
   ...
   write(errstring,  *) 'Observation value is ', obs, ' ensemble mean value is ', prior_mean
   write(errstring2, *) 'The observation has 0.0 error variance, and the ensemble members have 0.0 spread.'
   write(errstring3, *) 'These require inconsistent actions and the algorithm cannot continue.'
   call error_handler(E_ERR, 'obs_increment', errstring, &
