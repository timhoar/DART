...
use time_manager_mod,     only : time_type, set_time, get_time, print_time,   &
                                 ...
                                 operator(<=), operator(>=)
use ensemble_manager_mod, only : get_ensemble_time, ensemble_type, update_vars
...
! we are going to read/write the vars array
call update_vars(ens_handle)
...
ENSEMBLE_MEMBERS: do i = 1, ens_handle%my_num_copies
   ...
   if(async == 0) then
      ...
      ! this version of async needs to pass the target time in as an
      ! optional third argument to give the model the ability to advance
      ! multiple time steps in one go.  but that would break all existing
      ! calls to adv_1step() in existing model_mod files.  so this is currently
      ! commented out.  to use it, comment it in and build with a model_mod
      ! that has 3 arguments to the adv_1step() subroutine, and comment out
      ! the call to the error handler.

      write(errstring,*)'To use async 1, change code in obs_model_mod.f90::advance_state()'
      call error_handler(E_ERR,'advance_state','currently commented out for backwards compability.', &
