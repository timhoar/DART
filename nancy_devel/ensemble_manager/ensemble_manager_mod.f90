...
public :: init_ensemble_manager,      end_ensemble_manager,     get_ensemble_time,      &
          ...
          broadcast_copy,             write_to_vars,            write_to_copies,        &
          read_from_vars,             read_from_copies,         update_vars,            &
          update_copies
...
type ensemble_type
   ...
   logical                  :: verbose   ! say something if valid suspect?
   integer                  :: id_num
...
! by default, don't complain about unneeded transposes
ens_handle%verbose = .false.
...
subroutine broadcast_copy(ens_handle, copy, vars)

! find the global copy number specified, broadcast it to all pes
! returning it in the vars array.  it's an output on all pes.
...
if(copy < 1 .or. copy > ens_handle%num_copies) then
   write(errstring, *) 'Requested copy: ', copy, ' is > maximum copy: ', ens_handle%num_copies 
   call error_handler(E_ERR,'broadcast_copy', errstring, source, revision, revdate)
endif
...
if(my_pe == owner) then
   vars = ens_handle%vars(:, owners_index) 
   call broadcast_send(owner, vars)
else 
   call broadcast_recv(owner, vars)
endif
...
subroutine write_to_vars(ens_handle)
...
end subroutine write_to_vars

!-----------------------------------------------------------------

subroutine write_to_copies(ens_handle)
...
end subroutine write_to_copies

!-----------------------------------------------------------------

subroutine read_from_vars(ens_handle)
...
if (ens_handle%valid /= VALID_VARS .and. ens_handle%valid /= VALID_BOTH) then
   call error_handler(E_ERR, 'read_from_vars', &
        'last access not var-complete', source, revision, revdate)
endif

end subroutine read_from_vars

!-----------------------------------------------------------------

subroutine read_from_copies(ens_handle)
...
if (ens_handle%valid /= VALID_COPIES .and. ens_handle%valid /= VALID_BOTH) then
   call error_handler(E_ERR, 'read_from_copies', &
        'last access not copy-complete', source, revision, revdate)
endif

end subroutine read_from_copies

!-----------------------------------------------------------------

subroutine update_vars(ens_handle)
...
if (ens_handle%valid /= VALID_VARS .and. ens_handle%valid /= VALID_BOTH) then
   call error_handler(E_ERR, 'update_vars', &
        'last access not var-complete', source, revision, revdate)
endif
ens_handle%valid = VALID_VARS
...
subroutine update_copies(ens_handle)
...
if (ens_handle%valid /= VALID_COPIES .and. ens_handle%valid /= VALID_BOTH) then
   call error_handler(E_ERR, 'update_copies', &
        'last access not copy-complete', source, revision, revdate)
endif
ens_handle%valid = VALID_COPIES

end subroutine update_copies
...
! Error checking
if ((ens1%valid /= VALID_VARS .and. ens2%valid /= VALID_BOTH) .or. &
    (ens1%valid /= VALID_VARS .and. ens2%valid /= VALID_BOTH)) then
   call error_handler(E_ERR, 'duplicate_ens', &
        'last access not var-complete for both inputs', source, revision, revdate)
endif
...
! print out size info.  set to .false. to shut off; .true. to enable
if (ens_handle%verbose .and. do_output()) then
   print *, 'ensemble mgr: handle num: ',          ens_handle%id_num 
   print *, 'ensemble mgr: number of    copies: ', ens_handle%num_copies
   print *, 'ensemble mgr: number of    vars  : ', ens_handle%num_vars
   print *, 'ensemble mgr: number of my_copies: ', ens_handle%my_num_copies
   print *, 'ensemble mgr: number of my_vars  : ', ens_handle%my_num_vars
endif
...
if (ens_handle%valid == VALID_BOTH) then
   if (ens_handle%verbose) then
      write(errstring, *) 'ens_handle ', ens_handle%id_num
      call error_handler(E_MSG, 'all_vars_to_all_copies', &
           'vars & copies both valid, xpose not needed', source, revision, revdate, &
            text2=errstring)
   endif
   return
