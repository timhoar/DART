...
subroutine get_close_boxes(gc, base_loc, base_kind, locs, loc_kinds, &
   num_close, close_ind, dist)

! FIXME: these work on any locations. the names of these args should be:  
!   gc, base_loc, base_kind, locs, loc_kinds, ... 

type(get_close_type), intent(in)  :: gc
type(location_type),  intent(in)  :: base_loc, locs(:)
integer,              intent(in)  :: base_kind, loc_kinds(:)
integer,              intent(out) :: num_close, close_ind(:)
real(r8), optional,   intent(out) :: dist(:)
...
subroutine get_close_otree(gc, base_loc, base_kind, locs, loc_kinds, &
   num_close, close_ind, dist)
...
call collect_nearby(gc%root, base_loc, gc%maxdist, locs, base_kind, loc_kinds, &
                    num_close, close_ind, dist)
