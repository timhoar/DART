...
use ensemble_manager_mod, only : init_ensemble_manager, end_ensemble_manager,                &
                                 ...
                                 read_from_vars, write_to_vars, read_from_copies, write_to_copies
...
AdvanceTime : do
   ...
   if(do_single_ss_inflate(prior_inflate) .or. do_varying_ss_inflate(prior_inflate)) then
      ...
      if (inf_damping(1) /= 1.0_r8) then
         call write_to_copies(ens_handle)
...
do i = 1, get_num_qc(seq)
   get_obs_qc_index = i
print *, 'getting qc metadata for index ', i, 'which is: ', trim(get_qc_meta_data(seq, i))
...
print *, 'getting dqc metadata for index ', i, 'which is: ', trim(get_qc_meta_data(seq, i))
...
print *, 'getting blank metadata for index ', i, 'which is: ', trim(get_qc_meta_data(seq, i))
...
call write_to_vars(obs_ens_handle)
call write_to_vars(forward_op_ens_handle)
call read_from_vars(ens_handle)
   ...
   do k = 1, my_num_copies
      ...
      if(global_ens_index <= ens_size) then
         ...
         call get_expected_obs(seq, thiskey, &
            ...
            thisvar, istatus, assimilate_this_ob, evaluate_this_ob, isprior)
...
call read_from_copies(forward_op_ens_handle)
call write_to_copies(obs_ens_handle)
...
do j = 1, obs_ens_handle%my_num_vars
   ...
   ! and then make decisions based on the specific type (in spite of
   ! the name, get_obs_kind() returns the specific obs type, e.g. RADIOSONDE_TEMP
   ! and NOT KIND_TEMPERATURE).
   ...
   if(prior_post == PRIOR_DIAG) then
      ...
      if(do_outlier .and. (obs_ens_handle%copies(OBS_GLOBAL_QC_COPY, j) < 2)) then
         ...
         ! FIXME: could call subroutine from quality_control_mod here with:
         ! failed = check_outlier(ratio, outlier_threshold, this_obs_type)
         ! if (failed) then
