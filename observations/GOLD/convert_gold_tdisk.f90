...
program convert_gold_tdisk

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!   convert_gold_nmax - program that reads a GOLD netCDF TDISK
...
namelist /GOLD_tdisk_nc_to_obs_nml/  &
     gold_netcdf_file, &
     gold_out_file,    &
     debug
...
real(r8) :: height, qc, qerr, oerr, tdisk_miss
...
call initialize_utilities('convert_gold_tdisk')

! Read the DART namelist for this model
call find_namelist_in_file('input.nml', 'GOLD_tdisk_nc_to_obs_nml', iunit)
read(iunit, nml = GOLD_tdisk_nc_to_obs_nml, iostat = rcio)
call check_namelist_read(iunit, rcio, 'GOLD_tdisk_nc_to_obs_nml')
...
call nc_check( nf90_open(gold_netcdf_file, nf90_nowrite, ncid), &
               'convert_gold_tdisk', 'opening file '//trim(gold_netcdf_file) )
...
allocate( tdisk_unc_mod(nlon,nlat,nscan))
...
call nc_check( nf90_close(ncid), &
     'convert_gold_tdisk', 'closing file '//trim(gold_netcdf_file) )
...
scanloop: do k = 1, nscan
   lonloop: do l = 1, nlon
      latloop: do m = 1, nlat
         ...
         !oerr = (tdisk_unc_ran(l,m,k)**2.0 + tdisk_unc_sys(l,m,k)**2.0 + tdisk_unc_mod(l,m,k)**2.0)**0.5
...
end program convert_gold_tdisk
