...
if [[ ${INDIVIDUAL_MEMBERS} -eq true ]]; then export DELETE_TEMP_DIR=false; fi
#
# Define experiment parameters
export MYNAME=$0
...
# Find the next available WRFBDY file
   ...
   while [[ ${KEYS[${FILE}]} -le ${WRFKEY} ]]; do
      if [[ ${FILE} -lt ${#BDYFILES[@]} ]]; then
         ...
         echo No WRFBDY file time that exceed the forecast time
         echo ${START_STRING}
         exit
      fi
   done
...
#
# Run wrfchem
      mpirun.lsf ./wrf.exe > index.html 2>&1 
      export RC=$?
