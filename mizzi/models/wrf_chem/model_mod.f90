...
! $URL: https://subversion.ucar.edu/DAReS/DART/branches/development/models/wrf/model_mod.f90 $
! $Id: model_mod.f90 5771 2012-07-05 16:13:54Z nancy $
! $Revision: 5771 $
...
! Mods for IASI chemistry
                              IASI_O3_RETRIEVAL, MOPITT_CO_RETRIEVAL, MODIS_AOD_RETRIEVAL,      &
!
                              KIND_O3_COLUMN, KIND_CO, KIND_O3,         &
...
character(len=128), parameter :: &
   source   = "$URL: https://subversion.ucar.edu/DAReS/DART/branches/development/models/wrf/model_mod.f90 $", &
   revision = "$Revision: 5771 $", &
   revdate  = "$Date: 2012-07-05 10:13:54 -0600 (Thu, 05 Jul 2012) $"
...
! AFAJ ++
   integer :: type_o3_column, type_o3, type_co, type_no, type_no2, type_hno3, &
...
!         print *, "APM:k ",boundsCheck( k, .false.,                id, dim=3, type=wrf%dom(id)%type_co)     
   ...
   ! The 33 is hard wired for IASI.  Need to change for MOPITT AFAJ
            if (k < 33) then          
...
! AFAJ for log space, do not need the following for MOPITT
!            fld = max(0.0_r8, fld)
         endif
      endif
! AFAJ --
...
!          (base_obs_kind.ne.IASI_O3_COL_RETRIEVAL)      .and. &
!          (base_obs_kind.ne.IASI_O3_STRATCOL_RETRIEVAL) .and. &
          (base_obs_kind.ne.MOPITT_CO_RETRIEVAL)        .and. &
          (base_obs_kind.ne.MODIS_AOD_RETRIEVAL)        &
                                                ) then
         if ( (obs_kind(t_ind).ne.KIND_O3_COLUMN)       .and. &
...
!
              ...
              (obs_kind(t_ind).ne.KIND_TAUAER4)         &
                                               ) then
              dist(k) = dist(k)
         else
              dist(k) = 999999.9_r8
