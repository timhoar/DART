...
public :: location_type, get_dist, get_location, set_location, &
          set_location2, set_location_missing, location_inside, &
...
function location_inside(loc, minl, maxl)
!----------------------------------------------------------------------------
!
...
if ((minl%which_vert /= maxl%which_vert) .or. &
   ...
   call error_handler(E_ERR, 'location_inside', errstring, source, revision, revdate)
endif
