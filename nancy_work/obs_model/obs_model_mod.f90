...
integer,                 intent(in)    :: last_key_used, async, tasks_per_model_advance
...
if(time2 /= ens_time) then
   call advance_state(ens_handle, ens_size, time2, async, tasks_per_model_advance, &
                      adv_ens_command)
else
   call wait_if_needed(async)
endif
...
subroutine advance_state(ens_handle, ens_size, target_time, async, &
                         tasks_per_model_advance, adv_ens_command) 
...
integer,             intent(in)    :: ens_size, async, tasks_per_model_advance
character(len=*),    intent(in)    :: adv_ens_command
...
character(len = 129)                                   :: system_string, system_string2

type(time_type) :: time_step
integer         :: is1, is2, id1, id2, my_num_state_copies, global_ens_index
integer         :: i, control_unit, ic_file_unit, ud_file_unit, ens_in_list
integer         :: concurrent_task_count, sequential_advances, model_tasks
...
SHELL_ADVANCE_METHODS: if(async /= 0) then
   ...
   model_tasks = tasks_per_model_advance

   !---------------------------------------------------------------------------
   ! Each MPI task writes a control file and calls the model.  This expects that
   ! the model will be a single process.
   ...
   if (async == 2) then
      ...
      write(system_string2, '(     1x, i10)') model_tasks
     
      ! Issue a system command with arguments my_task, my_num_copies, and control_file
      call system(trim(adv_ens_command)//' '//trim(system_string)//' '//trim(control_file_name)//' '//trim(system_string2))
   ...
   !---------------------------------------------------------------------------
   ! Only MPI task 0 write a single control file listing every member in the
   ! ensemble, and then writes to a lock file to let the original job script
   ! start the model advances.  This is (too) tightly tied to code in the job
   ! script, and relies on being able to run multiple MPI jobs on the same set
   ! of nodes.
      ...
      ! if control file is still here, the advance failed.  only 0 really needs
      ! to check this, but if any others are laying around its also bad. 
      if(file_exist(control_file_name)) then
         write(errstring,*)'control file for task ',my_task_id(),' still exists; model advance failed'
         call error_handler(E_ERR,'advance_state', errstring, source, revision, revdate)
      endif
   ...
   !---------------------------------------------------------------------------
   ! On systems where one MPI job can start another, have every Nth MPI task
   ! write out a subset of the ensemble members, and call the advance script.
   ! async 5 calls the advance script directly from here while other tasks spin.
   ! async 7 is like async 4 in that it blocks and counts on an external job
   ! script starting up the MPI model advance program.

   else if (async == 5 .or. async == 7) then

      ! Every N tasks write out a subset of the ens names and start a script.
      ! The others just hang out and wait.  This version spins.

      ! FIXME: something here needs to be a namelist item, probably in filter since 
      ! the other related values are there, and then passed into this routine via args.
      ! should we ask users to say how many tasks per model?  or how many models to
      ! run.  i guess since we can change the number of overall tasks at will, it
      ! seems safer to say how many tasks per model advance and compute the rest.
      ! concurrent tasks must be rounded down; sequential advances rounded up.
 print *, 'tasks per model advance = ', model_tasks
      if (model_tasks <= 0) model_tasks = 1
      concurrent_task_count = (task_count()+model_tasks-1) / model_tasks
 print *, 'concurrent task count = ', concurrent_task_count
      if (concurrent_task_count <= 0) concurrent_task_count = 1
      sequential_advances = (ens_size+concurrent_task_count-1) / concurrent_task_count
 print *, 'sequential_advances = ', sequential_advances

      ! Every Nth task writes out a set of ic files.
      if (mod(my_task_id()+1, model_tasks) == 0) then
         ! Write the ic and ud file names to the control file
         control_unit = get_unit()
         open(unit = control_unit, file = trim(control_file_name))
         ! Write out the file names to a control file
         ...
         ! if it needs to create unique filenames for its own use.
         ens_in_list = 0
         do i = 1, ens_size, concurrent_task_count
            write(control_unit, '(i5)') i
            write(control_unit, '("assim_model_state_ic.", i4.4)') i
            write(control_unit, '("assim_model_state_ud.", i4.4)') i
            ens_in_list = ens_in_list + 1
         end do
         close(control_unit)
      endif

      ! make sure all tasks have finished writing their initial condition
      ! files before letting the process(es) start the model advances.
      call task_sync()
      ...
      if (async == 5) then
         ! try spinning until the model advance is done.
         if (mod(my_task_id()+1, model_tasks) == 0) then
            ! Arguments to advance model script are unique id and all ensemble copies
            write(system_string,  '(i10, 1x, i10)') my_task_id(), ens_in_list
            write(system_string2, '(     1x, i10)') model_tasks
     
            ! Issue a system command with arguments my_task, ens_size, and control_file
            call system(trim(adv_ens_command)//' '//trim(system_string)//' '//trim(control_file_name)//' '//trim(system_string2))
   
         endif

         ! make sure everyone waits for the advances to finish before reading 
         call task_sync()
      else
         ! kick the main script.  block until woken up.  this seemingly needs more
         ! info about who and how many things need doing, right?  write info to a
         ! separate file about ens size, and some subset of the 3 values computed above?
         call block_task()
      endif

      ! if any control file(s) are still here, the advance failed
      if(file_exist(control_file_name)) then
         write(errstring,*)'control file for task ',my_task_id(),' still exists; model advance failed'
         call error_handler(E_ERR,'advance_state', errstring, source, revision, revdate)
      endif
   ...
   !---------------------------------------------------------------------------
   ! On systems where one MPI job can start another, have MPI process 0 write out
   ! a single filter control file with all members, and then call the advance script
   ! directly here, just from task 0, while all other members spin until its done.
   ! Requires none of the handshaking with lock files that async 4 needs, but the
   ! other tasks spinning might compete for cycles with the model advances.

   else if (async == 6) then

      ! Only task 0 writes all ens names to file and starts script from pe0.
      if (my_task_id() == 0) then
         ! Write the ic and ud file names to the control file
         control_unit = get_unit()
         open(unit = control_unit, file = trim(control_file_name))
      ...
      if (my_task_id() == 0) then
         ! Arguments to advance model script are unique id and all ensemble copies
         write(system_string,  '(i10, 1x, i10)') my_task_id(), ens_size
         write(system_string2, '(     1x, i10)') model_tasks
      ...
      write(errstring,*)'input.nml - async is ',async,' must be 0, 2, 4, 5, 6, or 7'
      call error_handler(E_ERR,'advance_state', errstring, source, revision, revdate)
   endif
...
! if async = 0, 2: no synchronizing needed.  return now.
if ((async == 0) .or. (async == 2)) return

! either block or spin, but we need to sync up with the other tasks
! if they are going to wait in some fashion.  make sure not to try to
! read the ud files until everyone is done.

if (async >= 4 .and. async <= 7) then

   ! make sure all tasks have finished writing their initial condition
   ! files before starting the model advances.
   call task_sync()
   ...
   if (async == 4 .or. async == 5) call block_task()
 
   ! same as async 4 or 5, but try using a simpler spin loop to see how
   ! bad this is for performance.
   if (async == 6 .or. async == 7) call task_sync()
...
! (async 1 and 3 no longer exist in the current version of filter.)
write(errstring,*) 'input.nml - async is ',async,' must be 0, 2, 4, 5, 6, or 7'
