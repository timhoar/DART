 ...
 * in a loop for all of sept.   merges only gps files.
 * (see original geninput.c for one which merges with ncep obs)
 *
 * nsc 23jul08
 */
...
#if 0
! example of what needs to be generated:
&query_obs_seq_nml
   filename_seq = '/ptmp/nancy/ncep/obs_seq20070106'  /
...
{
    int i;
    int gregbase = 148408;    /* start of month */
    int startd = 1;
    int endd = 31;
    int month = 5;
    int year = 2006;
    ...
    for (i=startd; i<=endd; i++) {
        ...
        sprintf(buf, "                  \'../obs_seq.gpsro_%4d%02d%02d24\', ",
                year, month, i);
        addme(buf);
        sprintf(buf, "   filename_out = \'/ptmp/nancy/newgps/obs_seq.gpsro%4d%02d%02d\', ",
                year, month, i);
        addme(buf);
        ...
        sprintf(buf, "   filename_seq = \'/ptmp/nancy/newgps/obs_seq.gpsro%4d%02d%02d\'", 
                year, month, i);
        addme(buf);
        addme("   /");
