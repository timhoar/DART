...
! $URL$
! $Id: real_obs_mod.f90 3325 2008-04-30 20:17:26Z nancy $
! $Revision$
...
use time_manager_mod, only : time_type, operator(>), operator(<), &
                             operator(>=), operator(/=), set_date, &
                             set_calendar_type, get_time, get_date, set_time
use utilities_mod, only : get_unit, open_file, close_file, file_exist, &
                          register_module, error_handler, &
                          E_ERR, E_MSG, timestamp
use location_mod, only : location_type, set_location, &
                         VERTISPRESSURE, VERTISSURFACE, VERTISHEIGHT
