...
use         model_mod, only : static_init_model, model_interpolate, get_domain_info
...
!-------------------------------------------------------------------------------
   ...
   real(r8)                :: lat,lon,pressure, height, elevation, hsfc, xyz_loc(3), xloc, yloc

   integer       :: fid, var_id, rcode, nx, ny
!   real(r8), allocatable :: landmask(:,:), topo_hgt(:,:)
   character(len=80) :: name

   integer       :: i, levels, istatus, ii, jj, dom_id
   real(r8)      :: windspd, winddir, uu, vv, tk, td, rh, qv, altimeter
   real(r8)      :: obs_value(1), obs_err, obs_qc(1), xmod(1), topo_tol=100.0_r8

   logical       :: valid, init0=.true.

   if(init0 .eqv. .true.) then
   init0 = .false.
   call static_init_model

! open a wrfinput file, which is on this domain
!   rcode = nf90_open(path = "wrfinput_d01", mode = nf90_nowrite, ncid = fid)
!   rcode = nf90_inq_dimid(fid, "west_east", var_id)
!   rcode = nf90_inquire_dimension(fid, var_id, name, nx)
!   rcode = nf90_inq_dimid(fid, "south_north", var_id)
!   rcode = nf90_inquire_dimension(fid, var_id, name, ny)
!   allocate(landmask(nx,ny));  allocate( topo_hgt(nx,ny))

!   rcode = nf90_inq_varid(fid, "LANDMASK", var_id)
!   rcode = nf90_get_var(fid, var_id, landmask)

!  rcode = nf90_inq_varid(fid, "HGT", var_id)
!  rcode = nf90_get_var(fid, var_id, topo_hgt)
!  rcode = nf90_close(fid)
   ...
   if(lon < -1000.0_r8 .or. lon > 360.0_r8) go to 1000
   if( lat < -90.0_r8 .or. lat > 90.0_r8 ) go to 1000

   elevation = gtsobs%info%elv
   height = gtsobs%each%height%data
   pressure = gtsobs%each%pressure%data

   if (which_vert.eq.VERTISSURFACE ) then
      location = set_location(lon, lat, elevation, which_vert)
...
! liu   check model topography height and surfce station elevation. If the
!  difference is larger then 100 m, skip the observation.  xmod is not used
!
   if (which_vert .eq. VERTISSURFACE ) then

!  get the model topography height at the observation location
   call model_interpolate(xmod, location, KIND_SURFACE_ELEVATION, hsfc, istatus)
!
!  xyz_loc = get_location ( location )
!  call  get_domain_info (xyz_loc(1), xyz_loc(2), dom_id, xloc, yloc)
!  ii = ifix(xloc);  jj = ifix(yloc)
!
    if(abs(hsfc - elevation ) > topo_tol)  then
!    print*, 'tttt ', hsfc, elevation
     go to 1000
    endif
   endif
! liu
...
!     if ( valid .and. pressure >= 0.0_r8 .and. pressure < 105.0e3_r8 ) then
      if ( valid .and. pressure >= 85000.0_r8 .and. pressure < 105.0e3_r8 ) then
        altimeter = compute_altimeter( pressure*0.01, elevation )
!     print*, 'ttt= ',  altimeter, pressure*0.01, elevation
         obs_value(1) = altimeter     ! (hPa)
!        obs_value(1) = pressure
         obs_err = gtsobs%each%pressure%error * 0.01   !( hPa)
...
SUBROUTINE insert_gts_obs_multi(gtsobs, qc_threshold, &
                          pressure_type, height_type, u_wind_type,v_wind_type, &
                          temperature_type, dew_point_type, rh_type, thickness_type, &
                          which_vert, num_obs, obs, seq)
!-------------------------------------------------------------------------------------
   ...
   integer, optional, intent(in)          :: pressure_type, height_type, &
                                             u_wind_type, v_wind_type, &
                                             temperature_type, dew_point_type, &
                                             rh_type, thickness_type, qc_threshold
   integer, intent(in)                    :: which_vert
   integer, intent(inout)                 :: num_obs
   type(obs_type), intent(inout)          :: obs
   type(obs_sequence_type), intent(inout) :: seq
   ...
   integer                 :: i, levels
   real(r8)                :: windspd, winddir, uu, vv, tk, td, rh, qv, thickness
   real(r8)                :: obs_value(1), obs_err, obs_qc(1)

   logical                 :: valid, init0=.true.

   real(r8), allocatable, save :: landmask(:,:), topo_hgt(:,:)
   integer       :: fid, var_id, rcode, nx, ny
   character(len=80) :: name
   integer       :: istatus, ii, jj, dom_id
...
!liu
    if(lon < -1000.0_r8 .or. lon > 360.0_r8) go to 2000
    if( lat < -90.0_r8 .or. lat > 90.0_r8 ) go to 2000
    if(pressure < 15000.0) go to 2000
!liu
...
!   SATEM thickness -----------------
   if (present(thickness_type)) then
      !satem thickness in m, stored in temperature location

!   skip the satem observations above 100 hPa.
    if( i > 8 ) go to 2000

!  skip the data over land
   xyz_loc = get_location ( location )
   call  get_domain_info (xyz_loc(1), xyz_loc(2), dom_id, xloc, yloc)
   ii = nint(xloc);  jj = nint(yloc)
    if(dom_id >0 ) then
     if( landmask(ii,jj) >0.0 ) then 
      print*, 'skipped satem over land = ',  ii, jj , dom_id
      go to 2000
     endif
    endif
!
    if(i == 1) then 
!  add the surface layer thickness belwo 850 hPa.
      valid = .true.
      if (present(qc_threshold)) then
         if (qc_threshold .ge. 0 ) &
            valid =      (gtsobs%each(i)%temperature%qc .ge. 0) &
...
!    set pressure as the SLP
     pressure = gtsobs%loc%slp%data
    if(lon < -1000.0_r8 .or. lon > 360.0_r8) go to 2000
    if( lat < -90.0_r8 .or. lat > 90.0_r8 ) go to 2000
      location = set_location(lon, lat, pressure, which_vert)
     call set_obs_def_location(obs_def, location)

!    get the thickness between this pressure and one layer above.
      thickness = gtsobs%each(i)%temperature%data
      if ( valid .and. thickness > 0.0_r8 .and. thickness < 1.0e5_r8 ) then
         obs_value(1) = thickness
         obs_err = gtsobs%each(i)%temperature%error
         obs_qc(1)  = real(gtsobs%each(i)%temperature%qc,kind=r8)

         call set_obs_def_kind(obs_def, thickness_type)
         call set_obs_def_error_variance(obs_def, obs_err*obs_err)
         call set_obs_def(obs, obs_def)
         call set_obs_values(obs, obs_value)
         call set_qc(obs,obs_qc)
...
!  add the thickness above 850 hPa. The thickness is defined as the difference
!  between the specific levels, not as the ones relative to SLP.
      valid = .true.
      if (present(qc_threshold)) then
         if (qc_threshold .ge. 0 ) &
            valid =      (gtsobs%each(i+1)%temperature%qc .ge. 0) &
                   .and. (gtsobs%each(i+1)%temperature%qc .le. qc_threshold)
      endif
...
!    get the thickness between this pressure and one layer above.
      thickness = gtsobs%each(i+1)%temperature%data - gtsobs%each(i)%temperature%data

      if ( valid .and. thickness > 0.0_r8 .and. thickness < 1.0e5_r8 ) then
         obs_value(1) = thickness
         obs_err = gtsobs%each(i+1)%temperature%error
         obs_qc(1)  = real(gtsobs%each(i+1)%temperature%qc,kind=r8)
...
SUBROUTINE insert_gts_obs_gpsref(gtsobs, qc_threshold, &
                                 gpsref_type, gpsref_key, gpsref_form, &
                                 which_vert, num_obs, obs, seq)
!-------------------------------------------------------------------------------
   ...
   integer                 :: i, levels
   real(r8)                :: td, nx, ny, nz, ds, htop, rfict, thickness
   real(r8)                :: obs_value(1), obs_err, obs_qc(1)
...
!------------------
   ...
   if (valid) then
      ...
      height = gtsobs%each(i)%height%data
!liu
    if(height > 13000.0) go to 3000
    if(lon < -1000.0_r8 .or. lon > 360.0_r8) go to 3000
     if( lat < -90.0_r8 .or. lat > 90.0_r8 ) go to 3000
!liu
