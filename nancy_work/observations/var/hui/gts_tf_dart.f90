...
!liu
                              GPSRO_REFRACTIVITY, SATEM_THICKNESS
...
integer           :: num_obs, num_obs_last, num_copies, num_qc, max_num_obs=800000
...
!==================================================================
!liu  for satellite winds only change the observation error to NCEP
!  the values vary with seasons. The following is for June-sep?
          pressure = ob%satob(i)%each%pressure%data
        if(pressure > 70000.0) then
          ob%satob(i)%each%speed%error = 3.8
        elseif (pressure <=70000.0 .and. pressure >40000.0) then
          ob%satob(i)%each%speed%error = 5.0
        elseif ( pressure <=40000.0 )then
          ob%satob(i)%each%speed%error = 6.8
        endif

!   for may 2007, Jan 2007
!  obs_err = 1.8
!  obs_err = 3.0
!  obs_err = 5.0
!liu
...
if (Use_SynopObs .eqv. .TRUE.) then
   do i = 1, ob%num_synop
      call insert_gts_obs(ob%synop(i), &
                          qc_threshold = gts_qc_threshold, &
                          pressure_type    = LAND_SFC_ALTIMETER, &
!                         pressure_type    = SYNOP_SURFACE_PRESSURE, &
!   to use surface pressure only at the moment
...
if (Use_MetarObs .eqv. .TRUE.) then
   do i = 1, ob%num_metar
      call insert_gts_obs(ob%metar(i), &
                          qc_threshold = gts_qc_threshold, &
                          pressure_type    = MARINE_SFC_ALTIMETER, &
...
if (Use_gpsrefObs .eqv. .TRUE.) then
   ...
   do i = 1, ob%num_gpsref
      call insert_gts_obs(ob%gpsref(i), &
                          ...
                          gpsref_key = gpskey, gpsref_form = 1, &
