...
! $Source: /home/thoar/CVS.REPOS/DART/ncep_obs/real_obs_mod.f90,v $
! $Revision$
! $Date: 2006/07/21 22:32:18 $
! $Author$
! $Name:  $
...
character(len=128) :: &
source   = "$Source: /home/thoar/CVS.REPOS/DART/ncep_obs/real_obs_mod.f90,v $", &
revision = "$Revision$", &
revdate  = "$Date: 2006/07/21 22:32:18 $"
...
contains
  ...
  function real_obs_sequence (year, month, day, max_num, select_obs, ObsBase, &
           ADDUPA, AIRCAR, AIRCFT, SATEMP, SFCSHP, ADPSFC, SATWND, AIRX2R, &
...
!  selec the obs for the time window
  ...
  if(obs_prof == 1) then
    ...
    if(obstype == 173                    ) obs_kind = AIRS_TEMPERATURE
