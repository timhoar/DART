...
!------------------------------ 
  ...
  integer, parameter :: C_REAL4 = 4
  integer, parameter :: C_REAL8 = 8
  integer, parameter :: C_REAL = C_REAL8
...
!    character(len=len(str)),intent(out) :: lstr
  ...
  function uppercase(str) result(ustr)
    character(len=*), intent(in)       :: str
!    character(len=len(str)),intent(out):: ustr
  ...
  subroutine fix_for_platform4(array, array_size, element_size)
    real(kind=C_REAL4), dimension(:), intent(inout) :: array
