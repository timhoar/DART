...
character(len=128), parameter :: revdate  = "$Date$"
  ...
  real(kind=r8)       :: obs_loc_i, obs_loc_j
  real(kind=r8)       :: obs_lat, obs_lon
...
contains
  ...
  subroutine interpolate(state, grid, obs_loc, obs_kind, obs_value,&
                         interp_worked)
    ...
    call location_to_gridpt(grid, obs_loc, obs_loc_i, obs_loc_j,obs_loc_z)
    call gridpt_to_latlon(grid, obs_loc_i, obs_loc_j, obs_lat, obs_lon) 
    call check_ij_within_grid(grid, obs_loc_i, obs_loc_j, obs_within_grid)
    ...
    do i=1,NUM_NEIGHBORS
      call check_ij_within_grid(grid, real(neighbors_i(i),kind=r8), real(neighbors_j(i),kind=r8), &
                                      obs_within_grid)
      if (.not. obs_within_grid) then
        interp_worked = .false.
        return
    ...
    if(output_interpolation) call print_interpolation_diagnostics(obs_kind, obs_value)
  ...
  ! (geopotential height)
  !  PARAMETERS
  !   IN  obs_kind          integer representation of raw obs kind
  subroutine get_target_var(obs_kind)
    integer, intent(in) :: obs_kind
    ...
    select case (obs_kind)
      ...
      case(KIND_VERTLEVEL)
        col_values = spread(msigma_values, VALUES_DIM_NEIGHBOR, NUM_NEIGHBORS)
        vars_available(:,AVAILABLE_INDEX_TARGET) = .true.
      case default
        ! If we got here, we're observing something contained in
        ! the state vector directly.
        do cur_level = 1, num_model_levels
          call get_restart_index_by_properties(obs_kind, ISNT_MEAN,  &
          ...
          call destagger_field(var_field, obs_kind)
  ...
  subroutine destagger_field(field, field_type)
    ...
    if (field_type .eq. KIND_U_WIND_COMPONENT) then
      call utom(field, grid_i, grid_j, NUM_VERT_LEVELS, .true.)
    else if (field_type .eq. KIND_V_WIND_COMPONENT) then
      call vtom(field, grid_i, grid_j, NUM_VERT_LEVELS, .true.)
    end if
...
!   character(len=256) :: message
    ...
    real(kind=r8) :: scaled_level, max_minval, min_maxval
    ...
    min_maxval=minval(maxval(available_vcoord_vals,2))
    max_minval=maxval(minval(available_vcoord_vals,2))
    ! We're OK if the level is below the highest available and
    ! higher than the lowest available
    if (scaled_level .le. min_maxval .and.      &
        scaled_level .ge. max_minval     ) then
      level_in_range = .true.
    else
!      write (message,*) 'Trying to interpolate to vertical level', &
!                        scaled_level, 'but the available range is',&
!                        max_minval,'to',min_maxval
...
!                         trim(message), source, revision, revdate)
  ...
  subroutine check_for_missing_values(no_missing_values)
    ...
    num_missing_values = count(int(vinterp_values) == int(MISSING_VALUE))
  ...
  subroutine print_interpolation_diagnostics(obs_kind, obs_value)
    integer,       intent(in) :: obs_kind
    real(kind=r8), intent(in) :: obs_value
    
    if (do_output()) then
      call print_label_name('Interpolation Results')
      write (*,'(A15,T25,2(F15.6,1x))') 'I/J Coord     :', obs_loc_i, obs_loc_j
      write (*,'(A15,T25,2(F15.6,1x))') 'Lat/Lon Coord :', obs_lat,obs_lon
      write (*,'(A15T25F10.2)') 'Vertical      :', obs_loc_z
      write (*,'(A15T25I10)'  ) 'Vertical Type :', interpolate_to_level_type
      write (*,'(A15,T25,A)'  ) 'Variable Kind :', get_raw_obs_kind_name(obs_kind)
