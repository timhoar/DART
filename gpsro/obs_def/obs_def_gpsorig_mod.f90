...
! WARNING: this is the original version of the GPS observation forward
!  operator code.  If you are generating new obs_seq files, use the
!  obs_def_gps_mod.f90 file instead.  If you are trying to read in old
!  obs_seq files which contains obs in the original GPS format, use
!  this code.
...
!  use obs_def_gpsorig_mod, only : get_expected_gpsro_ref, interactive_gpsro_ref, &
...
namelist /obs_def_gpsorig_nml/ max_gpsro_obs
...
call find_namelist_in_file("input.nml", "obs_def_gpsorig_nml", iunit)
read(iunit, nml = obs_def_gpsorig_nml, iostat = rc)
call check_namelist_read(iunit, rc, "obs_def_gpsorig_nml")

! Record the namelist values used for the run ...
if (do_nml_file()) write(nmlfileunit, nml=obs_def_gpsorig_nml)
if (do_nml_term()) write(     *     , nml=obs_def_gpsorig_nml)
...
end module obs_def_gpsorig_mod
