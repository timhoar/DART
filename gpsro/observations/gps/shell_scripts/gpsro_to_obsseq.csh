...
# full pathname to DART/observations/gps/work directory
# if running this script from the observations/gps/shell_scripts dir
# the following line will work, else fill in full pathname.
setenv DART_WORK_DIR    `cd ../work; pwd`
##setenv DART_WORK_DIR   /home/${user}/DART/observations/gps/work
set cdaac_user    = username
set cdaac_pw      = password

# end of things which much be changed

# if you are in a hurry, this has no delay between requests:
##set wget_cmd           = 'wget -q -nd -np -r -l 10 -w 0'
# one second delay between requests to avoid hammering server:
...
# set to no to avoid some of the output from this script:
set chatty=yes
...
setenv CONV_PROG      convert_gps_to_obs
...
if ( $convert == 'yes') then
   ...
   rm -rf flist
   #rm -rf gpsro_gps_input.nc flist
