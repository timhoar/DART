...
# the script will try to link one from ${workdir} if it does not
# exist in this directory, and make a minimal input.nml as well.
...
# if advance_time does not exist in this dir, copy it over from
# the ../work directory, and create a dummy input.nml if none found.
if ( ! -e advance_time ) then
   cp -f ${workdir}/advance_time .
   if ( ! -e advance_time ) then
      echo "the advance_time program is not found in either the current"
      echo "directory or in ${workdir}. normally built in the gps/work"
      echo "directory by the quickbuild.csh script.  exiting with error."
      exit -1
   endif
endif

if ( ! -e input.nml ) then
   echo '# dummy for advance_time' >! input.nml
