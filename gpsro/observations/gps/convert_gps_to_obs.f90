...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!   convert_gps_to_obs - program that reads a COSMIC GPS observation 
...
use          types_mod, only : r8, pi, missing_r8, deg2rad, rad2deg
...
use   obs_def_mod,      only : obs_def_type, set_obs_def_time, set_obs_def_kind, &
                               ...
                               set_obs_def_key
use    obs_def_gps_mod, only : set_excess_phase
use       obs_kind_mod, only : GPS_RO_REFRACTIVITY, GPS_RO_EXCESSPHASE
...
character(len=128) :: gpsro_netcdf_file     = 'gpsro_input.nc'
character(len=128) :: gpsro_netcdf_filelist = 'gpsro_input_list'
character(len=128) :: gpsro_out_file        = 'obs_seq.gpsro'

namelist /convert_gps_to_obs_nml/ obs_levels, local_operator, ray_ds,   &
...
call find_namelist_in_file("input.nml", "convert_gps_to_obs_nml", iunit)
read(iunit, nml = convert_gps_to_obs_nml, iostat = io)
call check_namelist_read(iunit, io, "convert_gps_to_obs_nml")

! Record the namelist values used for the run 
if (do_nml_file()) write(nmlfileunit, nml=convert_gps_to_obs_nml)
if (do_nml_term()) write(     *     , nml=convert_gps_to_obs_nml)
...
do k = 2, nlevels
  if ( obs_levels(k-1) >= obs_levels(k) ) then
    call error_handler(E_ERR, 'convert_gps_to_obs',       &
...
if ( file_exist ) then
  ...
  do k = 1, num_copies
    call set_copy_meta_data(obs_seq, k, 'GPSRO observation')
  end do
  do k = 1, num_qc
    call set_qc_meta_data(obs_seq, k, 'GPSRO QC')
  end do
...
fileloop: do      ! until out of files
   ...
   obsloop2: do k = 1, nlevels
     ...
     if ( local_operator ) then
        ...
        call set_obs_def_kind(obs_def, GPS_RO_REFRACTIVITY)
   ...
   !          excess_obserr_percent(hghto * 0.001_r8), oerr
        call set_obs_def_kind(obs_def, GPS_RO_EXCESSPHASE)
        call set_excess_phase(obs_num, nx, ny, nz, rfict, ray_ds, ray_htop)
